...
- 当前模型（M5.2 起为 `mimo-v2.5`）在相同案例上独立运行 3 次，至少 2 次完整通过；历史模型结果不计入新模型分母。
- 失败运行同样保存，不从统计中排除。
...
- 默认开发模型：`mimo-v2.5`（M5.2 起；历史默认 `gemma4:e4b`、`qwen3.5:9b`）。
...
3. 当前模型（M5.2 起为 `mimo-v2.5`）独立运行 3 次，至少 2 次完整诊断正确。
4. 正确诊断必须同时满足：
...
- [ ] 当前模型（M5.2 起为 `mimo-v2.5`）完成真实工具调用和结构化输出。
- [ ] 单次运行预算被强制执行。
- [ ] Agent 证据不足时明确拒答。
