...
- 当前模型（M5.1 起为 `qwen3.5:9b`）在相同案例上独立运行 3 次，至少 2 次完整通过；`gemma4:e4b` 的历史结果不计入新模型分母。
- 失败运行同样保存，不从统计中排除。
...
- 默认开发模型：`qwen3.5:9b`（M5.1 起；历史默认 `gemma4:e4b`）。
...
历史记录：`gemma4:e4b` 在 M5 阶段触发本规则（工具调用后的结构化输出未能在冻结预算内稳定完成，详见旧实施计划与 `mistake.md`）；用户已于 2026-08-28 确认切换，本规则现适用于 `qwen3.5:9b`。
...
3. 当前模型（M5.1 起为 `qwen3.5:9b`）独立运行 3 次，至少 2 次完整诊断正确。
4. 正确诊断必须同时满足：
...
- [ ] Ollama 当前模型（M5.1 起为 `qwen3.5:9b`）完成真实工具调用和结构化输出。
- [ ] 单次运行预算被强制执行。
- [ ] Agent 证据不足时明确拒答。
